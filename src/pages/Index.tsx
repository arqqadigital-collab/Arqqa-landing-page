...
const LifelyPetsPage = ({ onBack }: { onBack: () => void }) => {
  return (
    <motion.div 
      initial={{ opacity: 0 }} 
      animate={{ opacity: 1 }} 
          ...
          <div className="max-w-4xl mx-auto text-center pt-20">
            ...
            >
              <p className="text-lg text-gray-300 font-light leading-relaxed">
                Lifely Pets was created from a genuine love for animals and a deep belief that pets are family. The brand is built on care, trust, and long-term commitment to animal health—values that needed to be reflected consistently across its digital presence.
              </p>
              <p className="text-lg text-gray-300 font-light leading-relaxed">
                As a growing e-commerce brand, Lifely required more than just an online store. The website needed to communicate emotional connection, educate customers, and deliver a seamless shopping experience that builds trust and drives repeat purchases.
              </p>
              <p className="text-lg text-gray-300 font-light leading-relaxed">
                ARQQA Agency partnered with Lifely Pets to audit, redesign, and develop a conversion-focused e-commerce platform—balancing emotional storytelling with performance, usability, and scalability.
              </p>
            </motion.div>
            ...
            >
              <div className="col-span-1 md:col-span-2 space-y-4">
                <span className="text-[10px] font-black text-red-600 uppercase tracking-[0.3em]">Solution & Execution</span>
    ...
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      {[...Array(15)].map((_, i) => (
        <motion.div
          ...
          style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, transform: `rotate(${Math.random() * 45}deg)` }}
        />
      ))}
    </div>
  );
...
// --- REUSABLE COMPONENTS ---
const PrimaryButton = ({ children, className = "", onClick, type = "button" }: { children: React.ReactNode; className?: string; onClick?: () => void; type?: "button" | "submit" }) => {
  return (
    <motion.button
      type={type}
      ...
      <div className="absolute bottom-8 left-8 right-8">
        ...
        >
          ...
          <div className="flex gap-2">
             <span className="px-3 py-1 rounded-lg bg-black/40 text-[9px] font-black text-gray-300 border border-white/5 uppercase tracking-widest">Structure</span>
             <span className="px-3 py-1 rounded-lg bg-black/40 text-[9px] font-black text-gray-300 border border-white/5 uppercase tracking-widest">Strategy</span>
          </div>
        </motion.div>
      </div>
    </div>
       ...
       <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] lg:w-[800px] lg:h-[800px] pointer-events-none flex items-center justify-center z-10 scale-75 md:scale-100">
          ...
          {[...Array(6)].map((_, i) => (
             <motion.div
                ...
                style={{ width: 200 + (i * 80), height: 200 + (i * 80), borderColor: i === 2 ? 'rgba(220,38,38,0.4)' : 'rgba(255,255,255,0.1)' }}
                animate={{ rotate: i % 2 === 0 ? 360 : -360 }}
                transition={{ duration: 30 + (i * 5), repeat: Infinity, ease: "linear" }}
             >
                <div className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 ${i === 2 ? 'bg-red-500' : 'bg-white'} rounded-full shadow-[0_0_15px_currentColor]`} />
             </motion.div>
          ))}
       </div>
    </div>
...
const ProcessPage = ({ onBack }: { onBack: () => void }) => {
  const steps = [
    ...
    { id: 3, title: "Onboarding & Kickoff", description: "Official project start. We set up communication channels and gather necessary assets.", details: ["Slack Channel Setup", "Asset Collection (Brand/Copy)", "Access to Analytics/Hosting"], icon: Rocket },
    { id: 4, title: "Architecture & Logic", description: "Before design, we map out the user flow and technical architecture to ensure scalability.", details: ["Sitemap & User Flow", "Wireframing Key Pages", "Tech Stack Finalization"], icon: Layers },
    { id: 5, title: "High-Fidelity Design", description: "We craft the visual interface, focusing on conversion psychology and brand aesthetics.", details: ["UI Design System", "Interactive Prototypes", "Design Review & Feedback"], icon: PenTool },
    { id: 6, title: "Development & QA", description: "Our engineers build the solution using clean, modern code, followed by rigorous testing.", details: ["Frontend & Backend Dev", "Performance Optimization", "Cross-Browser QA Testing"], icon: Code2 },
    { id: 7, title: "Launch & Growth", description: "We deploy your site and provide post-launch support to ensure everything runs smoothly.", details: ["Live Deployment", "SEO Indexing Submission", "30-Day Post-Launch Support"], icon: Globe }
  ];

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen bg-black text-white relative font-sans">
      <LiquidBackground />
      <div className="fixed top-0 left-0 right-0 z-50 p-6 flex justify-between items-center bg-black/80 backdrop-blur-md border-b border-white/10">
        <button onClick={onBack} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors">
          <ArrowLeft size={20} /><span className="text-sm font-bold uppercase tracking-widest">Back to Home</span>
        </button>
      ...
      <div className="container mx-auto px-6 py-32 relative z-10 max-w-4xl">
        <div className="text-center mb-20">
          <div className="inline-block mb-4 px-4 py-1.5 rounded-full bg-red-900/20 border border-red-600/30 text-red-500 text-xs font-black uppercase tracking-[0.2em]">The Protocol</div>
          <h1 className="text-5xl md:text-7xl font-black text-white mb-6 uppercase tracking-tighter italic">Arqqa <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-white">Onboarding.</span></h1>
        </div>
        <div className="relative">
          ...
          <div className="space-y-12">
            {steps.map((step, i) => (
              <motion.div key={step.id} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} className={`flex flex-col md:flex-row gap-8 items-start ${i % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                <div className={`flex-1 w-full ${i % 2 === 0 ? 'md:text-right' : 'md:text-left'} pl-16 md:pl-0`}>
                  <div className="bg-white/5 border border-white/10 p-8 rounded-3xl hover:border-red-600/30 transition-all group">
                     <span className="text-red-500 text-xs font-black uppercase tracking-widest mb-2 block">Phase 0{step.id}</span>
                     ...
                     <div className={`space-y-2 ${i % 2 === 0 ? 'items-end' : 'items-start'} flex flex-col`}>
                        {step.details.map((detail, idx) => (
                           <div key={idx} className="flex items-center gap-2 text-sm text-gray-500 group-hover:text-gray-300">
...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-black/90 backdrop-blur-xl border-b border-red-600/20 py-3' : 'bg-transparent py-6'}`}>
      <div className="container mx-auto px-6 flex items-center justify-between">
        <a href="#" className="flex items-center group">
          <img src={LOGO_URL} alt="Arqqa" className="h-7 md:h-12 object-contain drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] group-hover:drop-shadow-[0_0_12px_rgba(220,38,38,0.6)]" />
        </a>
        <div className="hidden md:flex items-center gap-10">
          {['Philosophy', 'Services', 'Process', 'Work'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-medium text-gray-400 hover:text-red-500 transition-all">{item}</a>
          ))}
          <PrimaryButton className="px-6 py-2.5 text-sm" onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}>Start a Project</PrimaryButton>
        </div>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="absolute top-full left-0 right-0 bg-black border-b border-red-900/40 p-6 flex flex-col gap-6 md:hidden backdrop-blur-2xl">
            {['Philosophy', 'Services', 'Process', 'Work'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMobileMenuOpen(false)} className="text-lg font-medium text-gray-300 hover:text-red-500">{item}</a>
            ))}
            <PrimaryButton className="w-full" onClick={() => { setMobileMenuOpen(false); document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' }); }}>Start a Project</PrimaryButton>
          </motion.div>
        )}
      </AnimatePresence>
    </nav>
...
const Hero = () => {
  ...
  return (
    <section ref={heroRef} className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden bg-black">
      ...
      <div className="container mx-auto px-6 relative z-20 text-center">
        <motion.div style={{ y: yText }} className="flex flex-col items-center">
          <h1 className="text-5xl md:text-9xl font-black tracking-tighter text-white mb-8 leading-[0.85] max-w-5xl mx-auto uppercase italic">
            ...
            <RevealLine delay={0.6}><span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 via-blue-500 to-red-600 animate-gradient-x">High-Impact</span></RevealLine>
            <RevealLine delay={0.8}>Digital Assets</RevealLine>
          </h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 1.2 }} className="text-lg md:text-2xl text-white/70 max-w-3xl mx-auto mb-12 leading-relaxed font-light italic">Custom websites engineered for scalability, trust, and aggressive lead generation.</motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 1.4 }}>
            <PrimaryButton onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}>Secure Your Strategy Call</PrimaryButton>
          </motion.div>
        </motion.div>
      </div>
    </section>
    ...
    <section className="py-12 border-y border-white/5 bg-black relative overflow-hidden z-20">
      <div className="container mx-auto px-6 flex flex-col md:flex-row items-center gap-8 md:gap-16">
        <div className="md:w-1/4 flex-shrink-0">
           <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-600 text-center md:text-left leading-relaxed">Trusted by <br className="hidden md:block" /> Industry Leaders</p>
        </div>
        <div className="md:w-3/4 w-full relative overflow-hidden">
           <motion.div className="flex gap-16 items-center w-max" animate={{ x: ["0%", "-50%"] }} transition={{ duration: 30, repeat: Infinity, ease: "linear" }}>
             {[...Array(2)].map((_, setIndex) => (
               <div key={setIndex} className="flex gap-16 items-center">
                 {["Stripe", "Klarna.", "Shopify", "Revolut", "Block", "Monzo", "Wise", "Coinbase"].map((name, i) => (
                   <span key={i} className="text-2xl md:text-3xl font-black text-white/20 uppercase italic tracking-tighter hover:text-white transition-colors">{name}</span>
                 ))}
               </div>
             ))}
           </motion.div>
  ...
  <section id="philosophy" className="py-32 relative bg-black overflow-hidden border-t border-white/5">
    <div className="container mx-auto px-6 relative z-10">
      <div className="grid md:grid-cols-2 gap-12 md:gap-20 items-center">
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-left">
          <span className="text-red-500 font-bold tracking-widest text-xs uppercase mb-6 block drop-shadow-[0_0_8px_rgba(220,38,38,0.5)]">Our Core Philosophy</span>
          <h2 className="text-4xl md:text-6xl font-bold text-white mb-8 leading-tight tracking-tight">Before you start… <br /><span className="text-red-600 drop-shadow-[0_0_20px_rgba(220,38,38,0.6)]">We Consult.</span></h2>
          <div className="space-y-6">
            <p className="text-gray-300 text-lg md:text-xl leading-relaxed font-light">Because the right scope matters more than anything else, we don't jump straight into building your website. We take the time to understand your business, your goals, and your real needs.</p>
            <p className="text-gray-400 text-base md:text-lg leading-relaxed font-light">Through a free consultation, we help you define what matters, what you need, and what you don't, from features to technical setup, so your website is built with clarity from day one.</p>
            <div className="pt-6 border-t border-white/10">
              <p className="text-white font-medium text-xl leading-snug">No unnecessary features. No overengineering. <br /><span className="text-red-500 font-bold">Just the right scope for your business.</span></p>
            </div>
          </div>
          <div className="mt-12">
             <PrimaryButton onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}>Book a Free Consultation</PrimaryButton>
          </div>
        </motion.div>
...
const Features = () => {
  const features = [
    { icon: MessageSquare, title: "Free consultation", desc: "To define the right scope needs", gradient: "from-orange-500 via-red-500 to-pink-500", glow: "rgba(255, 60, 0, 0.7)" },
    { icon: Layout, title: "Custom Design", desc: "Unique designs built according to your business", gradient: "from-blue-500 via-purple-500 to-pink-500", glow: "rgba(180, 0, 255, 0.7)" },
    { icon: Layers, title: "CMS Integration", desc: "Easy-to-manage systems for content control", gradient: "from-cyan-400 via-blue-500 to-indigo-600", glow: "rgba(0, 150, 255, 0.7)" },
    { icon: Search, title: "SEO Ready", desc: "Built-in tools ready to rank on search engines", gradient: "from-orange-500 via-red-500 to-pink-500", glow: "rgba(255, 60, 0, 0.7)" },
    { icon: Smartphone, title: "Mobile Responsive", desc: "Exceptional experience across all devices", gradient: "from-blue-500 via-purple-500 to-pink-500", glow: "rgba(180, 0, 255, 0.7)" },
    { icon: Globe, title: "Multilingual Options", desc: "Support for multiple languages", gradient: "from-cyan-400 via-blue-500 to-indigo-600", glow: "rgba(0, 150, 255, 0.7)" },
    { icon: ShieldCheck, title: "Support & Maintenance", desc: "2 months of free priority assistance", gradient: "from-orange-500 via-red-500 to-pink-500", glow: "rgba(255, 60, 0, 0.7)" },
    { icon: Zap, title: "Fast & Secure", desc: "Performance optimized and reinforced security", gradient: "from-blue-500 via-purple-500 to-pink-500", glow: "rgba(180, 0, 255, 0.7)" },
  ];
    ...
    <section id="services" className="py-32 relative bg-black overflow-hidden border-t border-white/5">
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <h2 className="text-4xl md:text-6xl font-bold text-white mb-6 tracking-tight uppercase tracking-tighter">What are you paying for?</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {features.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="relative group h-[280px]">
              <div className="absolute -inset-1 rounded-[2.5rem] opacity-0 group-hover:opacity-100 transition-all duration-500 blur-[45px] z-0 scale-110" style={{ backgroundColor: f.glow }} />
              <div className={`absolute inset-0 rounded-[2.5rem] p-[2px] bg-gradient-to-br ${f.gradient} transition-all duration-500 group-hover:scale-[1.03]`}>
                <div className="flex flex-col h-full w-full bg-[#080808] rounded-[2.4rem] p-8 relative z-10 overflow-hidden">
                    <f.icon size={28} className="text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.4)] mb-6 group-hover:scale-110 group-hover:drop-shadow-[0_0_20px_white] transition-all" />
                    <div className="mt-auto">
                        <h3 className="text-xl font-bold text-white mb-3 tracking-tight">{f.title}</h3>
                        <p className="text-gray-500 text-sm font-light group-hover:text-gray-200 transition-colors">{f.desc}</p>
                    </div>
...
const CaseStudies = ({ onOpenProject }: { onOpenProject: (id: string) => void }) => {
  const projects = [
    { id: 'lifelypets', title: "Lifelypets", img: "https://i.ibb.co/HTqWsWLz/lifelypets.png", color: "from-blue-600 to-blue-900" },
    { id: 'starlight', title: "Starlightdevelopments", img: "https://i.ibb.co/TxVqqSDt/starlight.png", color: "from-purple-600 to-indigo-900" },
    ...
    { id: 'banks', title: "Egyptianbanks", img: "https://i.ibb.co/v91Jk2J/banks.png", color: "from-indigo-600 to-purple-900" },
    { id: 'direct', title: "Directgroup", img: "https://i.ibb.co/VpBCwpd2/direct.png", color: "from-violet-600 to-fuchsia-900" },
    { id: 'clavigerme', title: "Clavigerme", img: "https://i.ibb.co/qMQttB32/clavigerme.png", color: "from-emerald-600 to-teal-900" },
    { id: 'goalngo', title: "Goalngo", img: "https://i.ibb.co/pj419n24/goalngo.png", color: "from-orange-600 to-red-900" }
  ];
  ...
  return (
    <section id="work" className="py-32 relative bg-black overflow-hidden border-t border-white/5">
      <div className="container mx-auto px-6 relative z-10">
        <div className="mb-20">
          <h2 className="text-4xl md:text-7xl font-black text-white uppercase tracking-tighter italic leading-[0.85] mb-6">Success <br /> <span className="text-red-600">Architectures.</span></h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {projects.map((p, i) => (
            <motion.div 
              ...
              whileHover={{ y: -5 }}
            >
              <div className={`absolute inset-0 bg-gradient-to-b ${p.color} opacity-60 group-hover:opacity-40 transition-opacity duration-700`} />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center mb-12">
          <div className="order-2 lg:order-1">
            <h2 className="text-4xl md:text-6xl font-bold text-white mb-6 uppercase tracking-tighter">Just trust <br /><span className="text-red-600">the process.</span></h2>
            <div className="space-y-3 mb-8"> 
              {steps.map((step, i) => (
                <div key={i} className="bg-white/[0.03] border border-white/10 p-3 rounded-xl flex items-center gap-4 group hover:bg-red-600 transition-all duration-300">
            ...
            <PrimaryButton className="w-full md:w-2/3 py-5 text-lg" onClick={() => { window.scrollTo(0,0); onNavigate(); }}>View Full Process</PrimaryButton>
          </div>
...
const Testimonials = () => {
  const testimonials = [
    { company: "Company", logo: Layers, text: "Real-time insights from AI agents help us spot trends before they emerge.", author: "Jamal Richardson", role: "Head of Analytics", img: "https://i.pravatar.cc/150?u=jamal", gradient: "from-blue-500 to-purple-500" },
    { company: "Logoipsum", logo: Globe, text: "Seamless Slack and Salesforce integrations keep our team in sync. Fusion AI is vital.", author: "Priya Desai", role: "Customer Success Manager", img: "https://i.pravatar.cc/150?u=priya", gradient: "from-orange-500 to-red-500" },
    { company: "Venture", logo: Zap, text: "Automated workflows have cut our manual data entry by 80%. Exceptional partner.", author: "Mat Mursmark", role: "Chief Growth Officer", img: "https://i.pravatar.cc/150?u=mat", gradient: "from-emerald-500 to-teal-500" },
    { company: "Apex", logo: BarChart3, text: "Scalability was our biggest concern, but this architecture handled 10x growth.", author: "Sarah Jenkins", role: "CTO", img: "https://i.pravatar.cc/150?u=sarah", gradient: "from-indigo-500 to-blue-500" }
  ];
  ...
  return (
    <section className="py-32 bg-black overflow-hidden relative border-t border-white/5">
      <div className="container mx-auto px-6 relative z-10 text-center mb-20">
        <h2 className="text-4xl md:text-6xl font-bold text-white mb-6 uppercase tracking-tighter">We Have Done This Before</h2>
      </div>
      <div className="relative flex overflow-hidden">
        <motion.div className="flex gap-8 py-10" animate={{ x: [0, "-50%"] }} transition={{ duration: 60, repeat: Infinity, ease: "linear" }}>
          {repeated.map((t, i) => (
            <div key={i} className="relative w-[350px] md:w-[400px] h-[450px] p-8 rounded-[2rem] bg-white/5 border border-white/10 flex flex-col justify-between overflow-hidden group shrink-0 backdrop-blur-3xl">
              <div className={`absolute bottom-[-10%] left-[-10%] w-[70%] h-[70%] bg-gradient-to-tr ${t.gradient} blur-[90px] opacity-40 rounded-full pointer-events-none`} />
              ...
              <div className="relative z-10 flex items-center gap-4 mt-8 pt-8 border-t border-white/10">
                  <img src={t.img} alt={t.author} className="w-12 h-12 rounded-xl object-cover grayscale ring-2 ring-white/10" />
                  <div>
    ...
    <section id="contact" className="py-32 relative overflow-hidden bg-black border-t border-white/5">
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="text-4xl md:text-7xl font-bold text-white mb-10 leading-tight uppercase tracking-tighter italic">Don't build <br/> before <br/> <span className="text-red-500 underline decoration-blue-900 underline-offset-8">defining scope.</span></h2>
          </div>
              ...
              <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <input type="text" placeholder="John Doe" className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 text-white focus:outline-none focus:ring-1 focus:ring-red-500/50" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1">Mobile Number</label>
                    <input type="tel" placeholder="+1 (555) 000-0000" className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 text-white focus:outline-none focus:ring-1 focus:ring-red-500/50" />
                  </div>
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1">Work Email</label>
                  <input type="email" placeholder="name@company.com" className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 text-white focus:outline-none focus:ring-1 focus:ring-red-500/50" />
                </div>
                <div className="space-y-3">
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1">Message</label>
                    <textarea rows={4} placeholder="Tell us about your project..." className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 text-white focus:outline-none focus:ring-1 focus:ring-red-500/50 resize-none"></textarea>
                </div>
                <div className="pt-6">
                  <PrimaryButton type="submit" className="w-full rounded-2xl py-5 uppercase font-black text-xs tracking-widest bg-gradient-to-r from-red-600 to-blue-600">Book My Strategy Call</PrimaryButton>
                </div>
              </form>
            </div>
          </div>
  ...
  <footer className="py-16 border-t border-white/10 bg-black relative z-10">
    <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-10">
      ...
      <div className="flex gap-8">
        {['Philosophy', 'Services', 'Process', 'Work'].map(s => <a key={s} href={`#${s.toLowerCase()}`} className="text-xs font-bold uppercase tracking-widest text-gray-600 hover:text-red-500 transition-colors">{s}</a>)}
      </div>
      <p className="text-gray-600 text-[10px] uppercase tracking-[0.3em] font-bold">Built for <span className="text-red-600">Clarity</span></p>
    </div>
  </footer>
);
